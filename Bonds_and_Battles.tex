...
\end{align}
If $\tau^{2} = 0$, then Equations (\ref{bonds_battles:eq:6}) and (\ref{bonds_battles:eq:7}) would be equivalent to simply estimating the a model with the first difference of $y$ as the dependent variable.
...
\end{equation}
Since Figure \ref{bonds_battles:fig:fives1874_price} suggests pretty strongly that the variance of the yields over time is not constant, the variance  $\sigma_{y^{*},t}$ is modeled as varying over time using a stochastic volatility process,
\begin{equation}
