...
These are observed at irregular frequencies, \ensuremath{\infty{}} to \ensuremath{\infty{}} days, but the estimation procedure handles missing values.
...
The matrix $W$ contains the weighted indictors of when news about a battle reaches the market.
...
News of battles is assumed to be present on the first day(s) news about the battl is reported in the \textit{New York Times} and the next 5 days.
...
For example, the end of Gettysburg was reported in the news on July 2nd--4th, 1862, so July 2, 1862 through July 9, 1862 have a weight of $\frac{1}{7}$, and all other days have a weight of 0.
...
\begin{align}
  \label{bonds:eq:10}
  \delta &= \alpha + X_{t} \begin{bmatrix}\beta_{C} & \beta_{U}\end{bmatrix} + W_{t} \gamma \\
  \label{bonds:eq:12}
...
In model $\Model{4}$, inthe random effects of the battles are shrunk towards the effect of the previous previous battle.%
\footnote{
