...
This work presents two implementations and and examples of this approach to change points.
...
While this one-group approach does not provide clear posterior probabilities that locations are change points, it is a reasonable model of change point processes for several reasons.%
...
\end{equation}
This means that this model is a GDLM as in Section \ref{sec:estimation}, and can be sampled with the method described in that setction.
