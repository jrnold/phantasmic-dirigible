...
Student's $t$: The mixing distribution is inverse gamma, $p(\lambda_{i}) = \dinvgamma{\xi / 2, \xi \tau^{2} / 2}$ where $\xi$ are the degrees of freedom. \parencite{Tipping2001}
Double Exponential (Laplacian): The mixing distribution is exponential \parencites{ParkCasella2008}{Hans2009}, 
\begin{equation}
  \begin{aligned}[t]
...
The horseshoe distribution \parencite{CarvalhoPolsonScott2009}{CarvalhoPolsonScott2010} is defined hierarchically as a scale-mixture of normals
\begin{equation}
  \label{eq:7}
  \begin{aligned}[t]
    \omega_{t} | \lambda_{t}, \tau & \sim \dnorm{0, \lambda_{t}^{2}} \\
    \lambda_{t} | \tau & \sim \dhalfcauchy{0, \tau}
  \end{aligned}
\end{equation}
...
\end{equation}
The Horseshoe distribution has some theoretically attractive properties for shrinkage and variable selection \parencite{CarvalhoPolsonScott2009}{CarvalhoPolsonScott2010}{DattaGhosh2012}{PasKleijnVaart2014a}.

The horseshoe+ distribution \parencite{} is similar to the Horseshoe distribution, but with an additional term,
\begin{equation}
  \label{eq:7}
  \begin{aligned}[t]
    \omega_{t} | \lambda_{t}, \eta_{t}, \tau & \sim \dnorm{0, \lambda_{t}^{2}} \\
    \lambda_{t} | \tau & \sim \dhalfcauchy{0, \tau \eta_{t}} \\
    \eta_{t} & \sim 
  \end{aligned}
\end{equation}
\textcite{BhadraDattaPolsonEtAl2015a} introduce the Horseshoe+ prior distribution to deal with ``ultra-sparse'' cases or


Equivalently, $\lambda_{i} \sim \dhalfcauchy{0, 1}$ 
Horseshoe+: 
