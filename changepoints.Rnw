...
\usepackage{hyperref}
\hypersetup{
  pdftitle={Bayesian Change Points and Linear Filtering in Dynamic Linear Models using Shrinkage Priors}
...
date_format <- function(x) gsub("\\b0+\\(\\d\\)", "\\1", format(x, "%B %d, %Y"), perl = TRUE)
...
In this case, replace Equation (\ref{eq:1}) with
\begin{equation}
  \label{eq:2}
  \begin{aligned}[t]
    \mu_{t} &= \mu_{t - 1} + \omega_{t}
...
Equation (\ref{eq:3}) is a so-called ``spike and slab'' prior \parencite{MitchellBeauchamp1988a}.
Since Equation (\ref{eq:3}) explicitly models the two groups of zero and non-zero parameters, \textcite{Efron2008a} calls this the two-group answer to the two-group problem.
...
This property means that many change point problems can be expressed as Gaussian dynamic linear models, and can make use of computationally efficient methods as discussed in Section \ref{sec:estimation}.
...
Equation (\ref{eq:4}) is the \textit{observation equation}, where $y_{t}$ (observation vector) is an $r \times 1$ vector of observed data, $\theta_{t}$ (state equation) is a $p \times 1$ vector of the latent states, $b_{t}$ is an $r \times 1$ vector, $F_{t}$ is a $r \times p$ matrix, and $V_{t}$ (observation variance) is an $r \times r$ covariance matrix.
Equation (\ref{eq:24}) is the \textit{state equation} equation, which relates the current latent states to their previous values; $g_{t}$ is a $p \times 1$ vector, $G_{t}$ is a $p \times p$ matrix, and $W_{t}$ (state variance) is an $p \times p$ covariance matrix.
...
The model in Equations \eqref{eq:17} and \eqref{eq:2} is a GDLM if $\epsilon_{t} \sim \dnorm{0, V}$ and $\omega_{t} \sim \dnorm{0, W_{t}}$.
...
First, it suggests how these models can be extended beyond the simple change in level model in Section \ref{sec:chang-as-vari}.
...
GDLMs can be directly estimated in \Stan{} by translating the model described by Equations (\ref{eq:4}) and (\ref{eq:24}) into a \Stan{} model.
...
Section \ref{sec:example-stan-program} provides the code for one of the \Stan{} models used in this paper which uses these functions.%
...
Figure \ref{fig:nile} plots this data.
...
Table \ref{tab:nile-model_comp} compares the models using several statistics.
...
Since this model is a GDLM, it can be efficiently sampled using the methods in \ref{sec:estimation}.


\section{Example: George W. Bush Approval Ratings}
\label{sec:george-w.-bush}

As an example of a time series that is a smooth curve with jumps \textcite{RatkovicEng2010} use the approval ratings for George W. Bush, displayed in Figure \ref{fig:bush_approval}.
...
The data used in this example consists of \Sexpr{nrow(BushApproval)} polls between \Sexpr{date_format(min(BushApproval[["end_date"]]))} and \Sexpr{date_format(max(BushApproval[["end_date"]]))} from the Roper Center Public Opinion \footnote{From \url{http://webapps.ropercenter.uconn.edu/CFIDE/roper/presidential/webroot/presidential_rating_detail.cfm?allRate=True\&presidentName=Bush\#.UbeB8HUbyv8}.}
...
This model corresponds to a GDLM with latent states $\alpha$ and $\beta$, and thus the partially collapsed Gibbs sampling method in Section \ref{sec:estimation} can be used to efficiently estimate it.
