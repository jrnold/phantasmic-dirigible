...
An alternative approach is to rewrite the problem in equation (\ref{eq:1}) can be rewritten to focus on the values of the changes $\mu_{t} - \mu_{t - 1}$, rather than the locations of the change points:
\begin{equation}
  \label{eq:2}
  \begin{aligned}[t]
    y_{t} &= \mu_{t} + \epsilon_{t} \\
...
These approaches propose continuous distributions with a large spike at zero and    Equation (\ref{eq:3})  spike at zero, and fat tails.
