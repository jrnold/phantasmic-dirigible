...
date_format <- function(x) format(x, "%J %d, %Y")
...
\section{Identifying Change Points}
...
\begin{table}[thbp]
  \centering
  \input{\Sexpr{file.path(tab_path, "nile-tab_mu_model_comp.tex")}}
  \caption{
    Root mean squared errors (RMSE) of each model to the estimate of $\mu$ from the \ModelII{Intervention}.
...
The data used in this example consist of \Sexpr{nrow(BushApproval)} polls between \Sexpr{date_format(min(BushApproval[["start_date"]]))} and \Sexpr{date_format(min(BushApproval[["start_date"]]))} from the Roper Center Public Opinion Archives.%
...
% \subsection{American Civil War Currencies and Bonds}
% \label{sec:civil-war-greenbacks}



%Comparison of Kalman filter FFBS implementations: \textcite{DurbinKoopmans2012}
...
%  LocalWords:  PetrisPetroneEtAl2009 DurbinKoopman2012 Cobb1978
