...
\caption[Yields on U.S. government and state bonds, \Sexpr{date_bdy(DATE_PRICES_START)}--\Sexpr{date_bdy(DATE_WAR_START_END)}] {Yields to maturity of U.S. government and state bonds, by bond, \Sexpr{date_bdy(DATE_PRICES_START)} to \Sexpr{date_bdy(DATE_WAR_START_END)}.}
...
The yields of the U.S. 6's of 1868 were between \Sexpr{round(prices_prewar_period_bond["U.S. 6's, 1868:Pre Panic of 1857", "ytm_min"], 2)} and \Sexpr{round(prices_prewar_period_bond["U.S. 6's, 1868:Pre Panic of 1857", "ytm_max"], 2)} percent.
...
\begin{figure}
  ...
  \caption[Implied probabilities of war initiation from each Southern and Border state bond]{Implied probabilities of the initiation of war calculated from each Southern and Border state bond, \Sexpr{DATEFMT_RANGE_START} to \Sexpr{DATEFMT_RANGE_END}.}
  \label{fig:prwar1}
\end{figure}
