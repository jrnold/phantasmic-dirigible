...
\footnote{
Greenback refers to serveral non-interest bearing notes issued by the U.S. as the currency during the war.
...
In many cases these loans also are have exotic features realative to government bonds issed today.
...
The fraction of the budget spent directly on the military by the Union fell over the course the war because the fraction spent on paying the principal and interest on its debtincreased from 20 (1862) to 36 (1865) percent of the budget.
...
Since the cash flows of U.S. government bonds are not directly related to a war outcome, in order to use yields as proxy for the expectation of a war outcome, war outcomes need to influence future cash flows, and war expecatations need to be the primary factor influencing the yield.
...
An increasing probability of U.S.\ victory would make bonds less risky (higher price; lower yield), while a increasing expectated war cost would make bonds more risky (lower price; higher yield).
...
However, Figure~\ref{fig:fives1874_yield} shows that while the yields of U.S. government bonds fluctuate throught in the war, they do not decrease over the entire course of the war.
...
This work uses \Sexpr{nrow(major_battles)} battles with the ``A'' military significance classification.%
\footnote{
...
\begin{table}
  ...
  \caption[List of the militarily significant battles of the American Civil War]{List of the \Sexpr{nrow(major_battles)} militarily significant battles of the American Civil War included in this analysis.}
  \label{tab:battles}
\end{table}
...
The data on the Fives consist of \Sexpr{data_m1[["n"]]} observations between \Sexpr{data_m1[["START_DATE"]]}, the first observation after the Battle of Fort Sumter and the official start of the war, and \Sexpr{data_m2[["END_DATE"]]}, the end of the month after Robert E. Lee surrendered after the Battle of Appomattox Court House.
These are observed at irregular frequencies, between \Sexpr{min(data_m1[["data"]][["y_diff"]][-1])} and \Sexpr{min(data_m1[["data"]][["y_diff"]][-1])} days, but the estimated models handles missing values without additional adjustments.
...
If $\tau^{2} = 0$, then Equations~(\ref{eq:6}) and~(\ref{eq:1}) would be equivalent to simply estimating the a model with the first difference of $\log y_{t}$ as the dependent variable.
...
\begin{align}
  ...
  W_{b,t} &= 
            ...
            \end{cases}
\end{align}
where $B$ is the set of all battles in the data (Table \ref{tab:battles}).
...
This means that the effect of the Battle of Gettysburg, $\omega_{\Sexpr{n_battle_gettysburg}}$, would be shrunk towards the average effect of Union victories, $\beta_{U}$, and the effect of the previous battle, $\omega_{\Sexpr{n_battle_gettysburg - 1}}$ (\Sexpr{n_battle_gettysburg_previous} on May 16, 1863).%
...
Table \ref{tab:avg_battle_effects} shows the summary statistics of the posterior distribution of the average effects for the models including battle-effects.
...
Figures \ref{fig:btl_effects} plots the battle effects, $\omega_{b}$, for each battle, for each model.
Figures \ref{fig:btl_effects_ordered} plots the battle effects for model $\Model{3}$, for each battle.
Since the battle effects in model $\Model{3}$ can be considered time-varying values of effect of Confederate and Union victories, Figures \ref{fig:btl_effects_time} plots the battle effects for Confederate and Union victories over time. 
The first overall takeaway of the models is that it is difficult to get precise estimates; in Figures \ref{fig:btl_effects_ordered}, most battles have 95 percent credible intervals which include zero.
