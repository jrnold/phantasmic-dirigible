...
Consdering that there are only \Sexpr{nrow(major_battles)} battles, but \Sexpr{as.integer(data_m2[["END_DATE"]] - data_m2[["START_DATE"]]) + 1L} values of $\eta_{t}$, neither of those seems particularly large.
...
A rough sketch of the analysis would be to use compuational natural language processing tools to extract topics from contemporary newspapers and regress those topics on market movements.
...
When the market received information woudl be the days on which those topics appeared in newspapers.
