...
While I motivate the use of financial asset prices to study war from the perspective of the bargaining theory of war, in this work, I do not attempt to directly test or distinguish between the private information and commitment theories of war in this work.%
...
Using the Fives of 1874 relative to other bonds should not make a substantive difference, since as Figure~\ref{fig:fives1874_union} shows, the yield to maturity of the Fives of 1874 is similar to the yields of all the other bonds issued by the U.S. government.
...
However, as with the Fives of 1874 versus other bonds,  Figure~\ref{fig:fives1874_greenbacks} shows that the prices of the Fives of 1874 in gold dollars is similar in both level and trend as the the price of Greenbacks in gold dollars, so the choice is unlikely to change the results.
...
These are observed at irregular frequencies, \Sexpr{min(data_m1[["y_diff"]][-1])} to \Sexpr{min(data_m1[["y_diff"]][-1])} days, but the estimation procedure handles missing values.
...
Equation~\eqref{eq1} states that the change in the latent log-yields, ($y^{*}_{t} - y^{*}_{t-1}$, is a function of changes in that periods, $\delta_{t}$, and mean-zero innovation, $\eta_{t}$.
...
\begin{align}
  \label{eq:2}
  \delta &= \alpha + X_{t} \begin{bmatrix}\beta_{C} & \beta_{U}\end{bmatrix}'  \\
...
In model $\Model{4}$, the effects of each battle, $\omega_{b}$ are both shrunk towards the average effect of its outcome and of the effect of previous previous battle of the same outcome.%
\begin{align}
  \label{eq:11}
...
This means that the effect of Gettysburg, $\omega_{\Sexpr{n_battle_gettsyburg}}$ would be shrunk towards the average effect of all Union victories, $(\beta_{U}$, and the effect of the previous battle, $\omega_{\Sexpr{n_battle_gettysburg - 1}}$ (\Sexpr{n_battle_gettysburg_previous}).%
...
Other work in 19th century economic history uses the interest rate of British consols as a risk free rate 
...
In the first step, I marginalize over the latent states of a state space model,  $y^{*2}$ in this case, and use HMC-NUTS to sample the other parameters.
...
Nor is this approach completely at odds with the type of analysis conducted in this work which uses expert-coded events; that additional information could be included in semi-supervised methods, e.g. structured topic modeling CITE.
