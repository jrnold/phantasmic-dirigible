...
The key feature of Equation (\ref{eq:3}) is that all of the price inputs are expectations conditioned on current information.
...
The yield to maturity of a coupon bond can be found by taking a known price and cash flow schedule in Equation (\ref{eq:3}) as solving for a constant discount rate.
...
Using the Fives of 1874 instead of these other bonds does not make a substantive difference, since as Figure~\ref{fig:fives1874_union} shows, the patterns of the yield to maturity of the Fives of 1874 is similar to the yields of all the other bonds issued by the U.S. government.
...
The proportions of the U.S. government revenues for each source of funding as follows: taxes \Sexpr{round(budget_min_pct["deficit"] * 100)}--\Sexpr{round(budget_max_pct["deficit"] * 100)}\% from loans, \Sexpr{round(budget_min_pct["receipts_taxes"] * 100)}--\Sexpr{round(budget_max_pct["receipts_taxes"] * 100)}\% from taxes, and \Sexpr{round(budget_min_pct["receipts_misc"] * 100)}--\Sexpr{round(budget_max_pct["receipts_misc"] * 100)}\% from miscellaneous sources (tariffs, land sales).
...
In 1860, the states that would make up the Confederacy accounted for \Sexpr{round((9103333 / 31443316) * 100, 0)} percent of the overall U.S. population,%
\footnote{
...
The striking difference in the trend of the prices of U.S. bonds and that of the Confederate currency (Graybacks) in Figure~\ref{fig:fives1874_compared} is also evidence that U.S. bond yields were less driven by expectations of U.S. victory than by the war cost.
...
However, Figure~\ref{fig:fives1874_yield} shows that while the yields of U.S. government bonds fluctuate early in the war, they do not decrease over the course of the war.
...
There are \Sexpr{data_m1[["n"]]} observations between \Sexpr{data_m1[["START_DATE"]]}, the first observation after the Battle of Fort Sumter and the official start of the war, and \Sexpr{data_m2[["END_DATE"]]}, the end of the month after Robert E. Lee surrendered after the Battle of Appomattox Court House.
Yields are observed at irregular frequencies, \Sexpr{min(data_m1[["data"]][["y_diff"]][-1])} to \Sexpr{min(data_m1[["data"]][["y_diff"]][-1])} days, but the models will use a latent variable that handles missing values without any additional adjustments.
In these models, I will use the logarithm of the yield since Figure \ref{fig:fives1874_yield_price} suggests that the variance of yields is increasing with price.
...
\end{align}
Equation~\eqref{eq:6} states that observed log-yields consist of a latent log-yield ($y^{*}_{t})$ and measurement error ($\epsilon_{t}$).
...
$B$ is the set of all battles in the data (Table \ref{tab:battles}).
...
This means that the effect of Gettysburg, $\omega_{\Sexpr{n_battle_gettysburg}}$ would be shrunk towards the average effect of Union victories, $\beta_{U}$, and the effect of the previous battle, $\omega_{\Sexpr{n_battle_gettysburg - 1}}$ (\Sexpr{n_battle_gettysburg_previous}).%
...
Unlike many analyses of bond yields, the models presented here do not include either measures of inflation or the risk-free interest rate.
...
For these models, the average effect of a Confederate victory is $\tilde{\beta}_{C} = \frac{1}{C}\sum_{b \in C} \omega_{b}$ and the average effect of a Union victory is $\tilde{\beta}\ = frac{1}{C}\sum_{b \in U} \omega_{b}$.

Table \ref{tab:avg_battle_effects} shows summary statistics for the average effects for the models including battle-effects.
...
\begin{figure}[!htpb]
  ...
  \caption[Battle effects for model $\Model{3}$, ordered by time]{
    Battle effects, $\omega_{b}$ for all battles for model $\Model{3}$, by battle result, and ordered by date of the battle.
  }
  \label{fig:btl_effects_time}
\end{figure}


Spotsylvania Court House, Second Petersburg, Cold Harbor
...
The Widely Appplicable Information Criterion (WAIC) \parencite{Watanabe2010} is an information criteria similar to AIC or the Deviance Information Criterion (DIC).
...
As operationalized, battles appear to improve the predictive power of the models, but that improvement is small.

The relatively small magnitude of the effects of individual battles is seen in Table \ref{tab:blt_effects_size}.
The average effect of a single Union victiory is much less than 1 standard deviation of the standard deviation of the system error $\eta$; Confederate victories are larger than than 1 sd of $\eta$, but less than 2 standard deviations.
Consdering that there are only \Sexpr{nrow(major_battles)} battles, but \Sexpr{as.integer(m2_data[["END_DATE"]] - m2_data[["START_DATE"]]) + 1L} values of $\eta_{t}$, neither of those seems particularly large.

There are two interpretations of the limited explanatory power of battles in this analysis.
First, this means that the combat processes either information about the war is 
Second, perhaps improvements in the models employed in this analysis would produce different results.
Perhaps could better model the time-series processes, or lag structure of battles, or I should have operationalized combat differently.
While I could make the models in this analysis more complicated, a better approach to distinguish between these interpretations is not more complicated models, but better data.
...
%  LocalWords:  diff HoffmanGelman2013 VanDykPark2008a 20's 40's 30's
%  LocalWords:  gettysburg WarrenPearson1933a BordoRockoff1996a WAIC
%  LocalWords:  RMSE Martin1886 GelmanCarlinSternEtAl2013a
