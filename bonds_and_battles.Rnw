...
\begin{equation}
  \label{eq:3}
  P_{t} = \frac{j = 1}{H} \frac{\E_{t + j}(C_{t + j} | \mathcal{I}_{t})}{1 + \E_{t}(d_{t+j} | \mathcal{I}_{t}}
\end{equation}
where $H$ is the number of cashflows, $C_{t + j}$ is the cash flow at time $t + j$, and $r_{d + j}$ is the discount rate for time $t + j$, and $\mathcal{I}_{t}$ is the information available to the market at time $t$.
...
The most important thing to note about the price in Equation (\ref{eq:3}) is that since all of its inputs are in the future and uncertain, it is an expectation conditioned on the current information available.
...
In this paper, instead of prices, I use the yields to maturity of coupon bond which are found by taking a known price and cash flow schedule in Equation (\ref{eq:3}) as solving for a single discount rate.
...
While there has been extensive work in modeling this war (CITES), the the core mechanisms of the theory has been unamenable to empirical, and especially quantitative, analysis.
...
Observing only battle results, such as victories or casualties, is insufficient to identify the casual mechanism of the war without relying on a well-specififed model of the war process.%
...
I start witht the observation that war is the combination of a political bargaining process and a military combat process. %
\footnote{%
...
In this paper, I use long-run interest rate on government debt is used as proxy for expectations of the the Civil War outcome for the U.S.
...
The yields of the 5's of 1874, a 5 percent coupon bond maturing in 1874, is the specific financial asset that I use as the measure of long-run U.S. government interest rates.
I use the 5's of 1874 over other U.S. government bonds, because they are only U.S. government bond that was both issued prior to the war and quoted throughout the war, meaning that it is the only complete time series from a single asset that spans the war.
...
Figures \ref{fig:fives1874_price} and \ref{fig:fives1874_yield} plot the price in gold dollars and yield to maturity of the 5's of 1874.
...
During the war the interest rate fluctuated between \Sexpr{format_yields(fives1874_yield_war[["min"]])} percent and \Sexpr{format_yields(fives1874_yield_war[["max"]])} percent, reaching its peak on \Sexpr{fives1874_yield_maxdate}.
...
The financial market data used in this work, as well as additional financial market data from the American Civil War period are provided in the dataset American Civil War Era Financial Data Collection (ACWFD) \parencite{Arnold2015a}.
This data collection contains bond prices and exchange rates published in the \textit{Bankers' Magazine} and \textit{Merchants' Magazine}, bid-level auction data, and miscellaneous financial and economic data for this period from a variety of primary and secondary sources.
...
First, The war was of such severity that the fiscal policies of the governments were essentially their military spending. % The primary expense to the belligerents was the military. %
For the Union, the War and Navy Departments accounted for 86--96 percent of expenditures in the budget \parencites[][259]{Ball1991}[see
also][354-355]{BurdekinLangdana1993}. %
For the Union, the War and Navy Departments accounted for 76 percent (in 1862) to 61 percent (in 1865) of the expenditures. %
The fraction of the budget spent directly on the military by the Union was not due to a larger fraction spent on civilian matters, but due to
a larger fraction spent on servicing its debt, 20 (1862) to 36 (1865) percent, the majority of which had been incurred to pay for the war \parencites{Treasury1861a}{Treasury1861b}{Treasury1862}{Treasury1863}{Treasury1864}{Treasury1865}.
\textcite[][668]{McCandless1996} asserts that ``[d]uring the war, the single most important indicator of future government expenditures is the process of the war itself.'' 
...
I suppose that the observed log yields are observed with measurment error,%
\footnote{Equation (\ref{eq:1}) is simplified for presentation. Since two sources are used, and in some cases there are multiple observations, $y_{t}$ is the mean of the observations, and $\tau_{t} = \tau / \sqrt{n_{t}}$ where $n_{t} \in \{1, 2\}$ is the number of obsevations on that day.}
\begin{align}
  \label{eq:6}
...
The rought sketch of the analysis would be to estimate topics from the text and regress those topics on market movements.
...
The importance of events relative to other events is identified through their prominence in the newspaperm, \eg{} if the front page is about a battle, then it is probably the news that is moving the market, whereas if the frontage is about the passage of a law or the economy, then that is probably the more important news.
