...
This ncreased debt would increase the possibility of default or the temptation of the government to repay its debt in currency rather than specie.
...
This suggests that U.S. governmen bond yields do not primarily reflecting expectations of U.S. victory, and are, instead, reflecting some other factor, which is likely war costs.
...
The are \Sexpr{data_m1[["n"]]} observations between \Sexpr{data_m1[["START_DATE"]]}, the first observation after the Battle of Fort Sumter and the official start of the war, and \Sexpr{data_m2[["END_DATE"]]}, the end of the month after Robert E. Lee surrendered after the Battle of Appomattox Court House.
These are observed at irregular frequencies, \Sexpr{min(data_m1[["data"]][["y_diff"]][-1])} to \Sexpr{min(data_m1[["data"]][["y_diff"]][-1])} days, but the estimation procedure handles missing values.
In the models, I use the logarithm of the yield since Figure \ref{fig:fives1874_yield_price} suggests that the variance of yields is increasing with price.
...
\end{align}
Equation~\eqref{eq:6} states that observed log-yields are observed with error, and $y^{*}$ is a latent variable representing the ``true'' log-yields.
Equation~\eqref{eq:1} states that the change in the latent log-yields, ($y^{*}_{t} - y^{*}_{t-1}$, is a function of changes in that periods, $\delta_{t}$, and mean-zero innovation, $\eta_{t}$.
The innovation, $\eta_{t}$, is distributed Student's $t$ to allow for large events and changes that are not modeled in $\delta_{t}$.
If $\tau^{2} = 0$, then Equations~(\ref{eq:6}) and~(\ref{eq:1}) would be equivalent to simply estimating the a model with the first difference of $y$ as the dependent variable.
...
News of battles is assumed to be present on the days the first news of the battle is reported in the \textit{New York Times} and the next \Sexpr{data_m2[["LAG"]]} days.
...
This means that the effect of Gettysburg, $\omega_{\Sexpr{n_battle_gettysburg}}$ would be shrunk towards the average effect of all Union victories, $(\beta_{U}$, and the effect of the previous battle, $\omega_{\Sexpr{n_battle_gettysburg - 1}}$ (\Sexpr{n_battle_gettysburg_previous}).%
...
Figures \ref{fig:btl_effects_ordered} plots the battle effects, $\omega_{b}$, for only 
...
\begin{table}[!htpb]
  ...
  \caption{
    Predictive criteria for each Widely Applicable Information Criterion (WAIC).
    RMSE is the root mean squared error of each model.
    $\text{elpd}_{\text{WAIC}}$ is the expected log posterior predictive density implied by the Widely Applicable Information Criterion (WAIC).
  }
  \label{tab:waic}
\end{table}
