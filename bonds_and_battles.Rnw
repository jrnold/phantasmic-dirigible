...
Since \parencite{Fearon1995}, the the dominant theory of war has been the bargaining theory of war; see \textcites{Reiter2003}{Powell2009}{Reiter2009} for discussions.%
...
\footnote{
  War can also occur due to the sides the being risk loving, which \textcite{Fearo1995} dismisses as implausible, and due to domestic politics.
...
So while empirical evidence for the bargaining theory of war can come from studying war onset, testing the ``core of the theory'' parencite requires intrawar data and as such has demanding data requirements, as such has been subject to few empirical studies \parencites{Reiter2003}{Ramsay2008}{Reiter2008}{Weisiger2015}.
...
First, financial asset price data allows for within-case quantitative analysis of war termination by using the price of financial assets as time-varying expecations or predictions of the war termination.
...
However,  it is difficult to necessarily know what is surprising to contemporaries, see \textcite{Shirkey2009} for an attempt to code surprising events.
...
Since a complete and coherent theory requires that combat solve the frictions that prevented a negotiated settlement without war \parencite{LeventogluSlantchev2007}, these causal mechanisms are the two identified in \textcite{Fearon1995} and \textcite{Powell2007}: private information and commitment.%
\footnote{Indivisibility, identified as a third mechanism in \textcite{Fearon1995}, is considered a special case of the commitment problem in \textcite{Powell2007}.}
...
The striking difference in the trend of the prices of U.S. bonds and that of the Confederate currency (Graybacks) in Figure \ref{fig:fives1874_compared} is evidence that U.S. bond yields were not driven by expectations of U.S. victory, and, by elimination, likely represent the cost of war to the U.S.
...
However, Figure \ref{fig:fives1874_yield} shows that while the yields of U.S. government bonds fluctuates early in the war, they do not decrease over the course of the war.
...
The battles used in the analysis are listed in Table \ref{tab:battles}.
...
This work will use  \Sexpr{nrow(major_battles)} battles with the ``A'' military significance classification.%
\footnote{
...
While this is a subset of the battles, it is still a relatively large number of battles by most war standards and similar in number and composition as the list of battles in other sources: \textcite{Livermore1900} includes \Sexpr{n_battles_sources[["livermore"]]} battles, \textcite{Bodart1908} includes \Sexpr{n_battles_sources[["bodart"]]}, \textcite{cdb90} includes \Sexpr{n_battles_sources[["CDB90"]]}.
...
The data used in this paper includes \Sexpr{union_victories} Union victories, \Sexpr{confederate_victories} Confederate victories, and two ``Inconclusive'' battles.
...
\begin{align}
  \label{eq:6}
  y_{t} &= y^{*}_{t} + \epsilon_{t} & \epsilon_{t} \sim \dnorm{0, \tau^{2}}
\end{align}
...
\begin{align}
  \label{eq:7}
  y^{*}_{t} &= y^{*}_{t - 1} + \alpha_{t} + W_{t} \beta + \upsilon + \omega_{y^{*},t}
\end{align}
If $\tau^{2} = 0$, then Equations (\ref{eq:6}) and (\ref{eq:7}) would be equivalent to simply estimating the a model with the first difference of $y$ as the dependent variable.
With a non-zero $\tau$, this is equivalent to a constrained ARIMAX(0, 1, 1).

The parameter $\beta$ is a vector containing the effect of each battles.
These parameters are modeled as coming from two distributions: Union victories, and Confederate victories:
\begin{equation}
  \label{eq:4}
  \begin{aligned}[t]
  \beta_{b} &\sim
  \begin{cases}
  \dnorm{\upsilon, \zeta} & \text{if $b$ is a Union victory} \\
  \dnorm{\kappa, \zeta} & \text{if $b$ is a Confederate victory} \\
  \end{cases}
  \end{aligned}
\end{equation}
where $\upsilon$ is average effect of a Union victory and $\kappa$ is the average effect of a Confederate victories.
Note that as $\zeta \to 0$, this model approaches simply including indicator variables for Confederate and Union victories.
Modeling the effects of battles in this way allows for the estimation of the average effects of Union and Confederate victories, while allowing 

The effect of a battle at time $t$ is the sum of the product of a daily weight term $w_{b,k}$ ($\sum_{t \in 1:T} w_{b,t} = 1$) and its overall effect $\beta_{b}$.
...
The trend term has a constant account for unobserved trends in the conduct of the war that are not picked up by the observed variables.
\begin{align}
  \label{eq:8}
  \alpha_{t} &\sim \dnorm{\rho_{\alpha} \alpha_{t - 1}, \sigma_{\alpha}^{2}}
\end{align}

The trend and level errors are correlated as suggested in \textcite{WestHarrison1997},
\begin{equation}
  \begin{bmatrix}
    \omega_{y^{*}} \\
    \omega_{\alpha^{*}}
  ...
  \dnorm{
    0,
    \begin{bmatrix}
      \sigma^{2}_{y^{*}, t} + \sigma_{\alpha}^{2} & \sigma_{\alpha}^{2} \\
      \sigma_{\alpha}^{2} & \sigma_{\alpha}^{2}
    \end{bmatrix}
  }
\end{equation}
Since Figure \ref{fig:fives1874_price} suggests pretty strongly that the variance of the yields over time is not constant, the variance  $\sigma_{y^{*},t}$ is modeled as varying over time using a stochastic volatility process,
\begin{equation}
  \label{eq:9}
  \begin{aligned}[t]
    \log \sigma_{y^{*}} &\sim \dnorm{\bar{\sigma}_{y^{*}} + \rho_{\sigma_{y^{*}}} \log \sigma_{y^{*}, t - 1}, \xi^{2}}
  \end{aligned}
\end{equation}

The model is estimated using MCMC methods to sample from the posterior distribution.
The MCMC sampling methodology employed is a NUTS-HMC algorithm as implemented in the probabilistic programming language \Stan{} \parencites{Stan2015a}.
I found that even in the HMC context, when estimating state space models, it is useful for both efficiency and speed to sample in two steps: (1) marginalize over the latent states using a Kalman filter and sample the other parameters using HMC, and (2) given the parameters in step 1, sample the latent spaces using a backward sampler.
This constitutes a partially collapsed Gibbs sampler (CITE).
4 chains were run and convergence assessed by multiple-chain R hat statistics.
