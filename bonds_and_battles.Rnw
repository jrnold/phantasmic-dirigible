...
Intro from old paper.

Hard to study intra-war events. Prices may be useful.

Summarize results.

In estimating this, several methodological issues arose with regard to event-type studies that use battle data, and I conclude by proposing that newspaper or other text should be used to generate events in this setting.
...
Considering that there are only \Sexpr{nrow(major_battles)} battles, but \Sexpr{as.integer(data_m2[["END_DATE"]] - data_m2[["START_DATE"]]) + 1L} values of $\eta_{t}$, neither of those seems particularly large.
...
%  LocalWords:  GelmanHwangVehtari2014a GelmanVehtari2014a 1L
