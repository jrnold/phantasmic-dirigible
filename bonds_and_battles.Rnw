...
Understanding what leads to the termination of war can help to understand the causes of war \parencite[x]{Blainey1988}.
But undestanding  \textcite{Gartner1998} calls ``opening up the black box of war.''
Carl von Clausewitz famously described war as ``simply a continuation of political inter-course, with the addition of other means \parencite[605]{Clausewitz 1989}.''
This statement has traditionally been used to remind the reader that war is not an in itself, but a means to a political end. However, that insight is well integrated into current international relations theories of war. War is thought to consist of both bargaining and fighting, and the theoretical puzzle is why, given that war is costly, then why do states use this ex post inefficient method to come to an agreement. 
How do the events within war, such as battles, lead to the termination of war?

Opening the ``black box of war'' has been difficult for quantitative analysis since the relevant data on intra-war is sparse \parencite{Reiter2003}{Reiter2009}.
Data on battles is not readily available for large numbers of wars.
...
The approach of using financial markets to better understand war termination is a general one that can used in the study of conflict.
Understanding theories of war initiation and termination requires an understanding of the means of war, military combat.

However, the empirical study of military combat within war has lagged behind the study of war initiation that treats war as a black box 
...
It also suggests that U.S. victories were expected, while Confederate victories were surprising.
...
The key feature of Equation \eqref{eq:3} is that all of the price inputs are expectations conditioned on current information.
...
The yield to maturity of a coupon bond can be found by taking a known price and cash flow schedule in Equation (\ref{eq:3}) and solving for a discount rate.
...
\end{equation}
Equation (\ref{eq:5}) shows that the interest rate moves inversely to the price: a higher yield corresponds to a lower price, and vice-versa.
...
Figure~\ref{fig:combat-causal-diagram} illustrates the relationship between means, mechanisms ans war outcomes.
...
\begin{figure}[!htpb]
  ...
  \label{fig:combat-causal-diagram}
\end{figure}
...
Figures~\ref{fig:fives1874_price} and~\ref{fig:fives1874_yield} plot the price in gold dollars and the yield to maturity of the Fives of 1874.
Prior to mid-1860, the yield was low with little variation, fluctuating between \Sexpr{format_yields(fives1874_yield_pre_war[["min"]])} and \Sexpr{format_yields(fives1874_yield_pre_war[["max"]])} percent.%
\footnote{
...
After Lincoln's election in November 1860,  the interest rate rose to \Sexpr{format_yields(fives1874_yield_post_election[["mean"]])} percent.
After the initiation of the war with the Battle of Sumter (April 12--14, 1861), the yield rose to \Sexpr{format_yields(fives1874_yield_post_sumter)} percent.
During the war, between April 1861 and April 1865, the yield fluctuated between \Sexpr{format_yields(fives1874_yield_war[["min"]])} and \Sexpr{format_yields(fives1874_yield_war[["max"]])} percent, reaching its peak on \Sexpr{fives1874_yield_maxdate}.
After the end of conflict in April 1865, the interest rate returned to stability at \Sexpr{format_yields(fives1874_yield_post_war[["mean"]])} percent, a rate higher than its pre-war level.
...
Using the Fives of 1874 instead of these other bonds does not make a substantive difference for the analysis, since as Figure~\ref{fig:fives1874_union} shows, the patterns of the yields  of the Fives of 1874 is similar to the yields of all the other U.S. government bonds.
...
Apart from the loss of observations, using Greenbacks instead of the Fives of 1874 would not make a substantive difference, since as Figure~\ref{fig:fives1874_greenbacks} shows, the price of Fives of 1874 in gold dollars is similar in both level and trend as the the price of Greenbacks in gold dollars.
...
The proportions of the U.S. government revenues for each source of funding was as follows: \Sexpr{round(budget_min_pct["deficit"] * 100)}--\Sexpr{round(budget_max_pct["deficit"] * 100)}\% from loans, \Sexpr{round(budget_min_pct["receipts_taxes"] * 100)}--\Sexpr{round(budget_max_pct["receipts_taxes"] * 100)}\% from taxes, and \Sexpr{round(budget_min_pct["receipts_misc"] * 100)}--\Sexpr{round(budget_max_pct["receipts_misc"] * 100)}\% from miscellaneous sources (tariffs, land sales).
...
In 1860, the states that would make up the Confederacy accounted for only \Sexpr{round((9103333 / 31443316) * 100, 0)} percent of the overall U.S. population,%
\footnote{
...
The default on U.S. government bonds could have taken several forms.
The government could stop paying interest payments, as the Southern states did at the start of the war \parencite[159]{BankersMagazine1862}[810,947]{BankersMagazine1860}.
After the U.S. started issuing greenbacks which were not redeemable on demand in gold, it could have paid off the 
There was a minor controversy among investors regarding a different bond issue, the 5-20's which were explicit on repaying the interest in specie but were ambiguous in how the interest would be repaid \parencite{Roll1972}.
...
Since these were issued after the suspension of convertability of Greenbacks to gold this may have not been a surprise to investors, but it suggests that this was a possibility. 
That the yields on U.S. government bonds when calculated in gold rose during the war, while those calculated in currency remained low and steady near 5 percent \parencite[305]{HomerSylla20015}, suggests that to the extent that investors viewed U.S. government bonds as risky it was that they would be repaid in depreciated currency rather than specie.%
If investors feared a complete default, then the yields calculated in currency should have also risen.
This is also an important reason for using yields calculated in gold, which did respond to war risk, rather than yield calculated in currency as in \textcite{HomerSylla2005}, which were much less sensitive to that risk.
...
The striking difference in the trend of the prices of U.S. bonds and that of the Graybacks (Confederate currency) in Figure~\ref{fig:fives1874_compared} is also evidence that U.S. bond yields were less driven by expectations of U.S. victory than by the war cost.
...
However, Figure~\ref{fig:fives1874_yield} shows that while the yields of U.S. government bonds fluctuate throughout the war, they do not decrease over the entire course of the war.
...
At the height of the the Panic of 1857, the yield of U.S. government bonds (Sixes of 1868) only reached \Sexpr{format_yields(yield_panic_1857)} percent.%
...
The battles used in this analysis are listed in Table~\ref{tab:battles}.
...
This work uses \Sexpr{nrow(major_battles)} battles with an ``A'' military significance classification.%
\footnote{
...
\textcite{Livermore1900} includes \Sexpr{n_battles_sources[["livermore"]]} battles, \textcite{Bodart1908} includes \Sexpr{n_battles_sources[["bodart"]]}, and \textcite{cdb90} includes \Sexpr{n_battles_sources[["CDB90"]]}.
...
The data used in this work includes \Sexpr{union_victories} Union victories, \Sexpr{confederate_victories} Confederate victories, and no ``Inconclusive'' battles.%
\footnote{
...
Figure~\ref{fig:major_battles} plots the date and outcomes of these battles over time.
...
\begin{table}
  ...
  \caption{List of the \Sexpr{nrow(major_battles)} militarily significant battles of the American Civil War included in this analysis.}
  \label{tab:battles}
\end{table}
...
The Fives data consistent of \Sexpr{data_m1[["data"]][["n"]]} observations between \Sexpr{data_m1[["START_DATE"]]}, the first observation after the Battle of Fort Sumter and the official start of the war, and \Sexpr{data_m2[["END_DATE"]]}.
These occur at irregular frequencies, with gaps of \Sexpr{min(data_m1[["data"]][["y_diff"]][-1])}--\Sexpr{min(data_m1[["data"]][["y_diff"]][-1])} days between observations.
In these models, I use the logarithm of the yield since Figure \ref{fig:fives1874_yield_price} suggests that the variance of yields is increasing in its level.
...
\end{align}
for days $t = 1, \dots, T$.
Equation~\eqref{eq:6} states that the observed log-yields consist of a latent log-yield ($y^{*}_{t})$ and measurement error ($\epsilon_{t}$).
Equation~\eqref{eq:1} states that the first difference in the latent log-yields, $y^{*}_{t} - y^{*}_{t-1}$, is the sum of $\delta_{t}$ and a mean-zero innovation, $\eta_{t}$.
...
If $\tau^{2} = 0$, then Equations~(\ref{eq:6}) and~(\ref{eq:1}) would be equivalent to simply estimating a model with the first difference of $\log y_{t}$ as the dependent variable.
...
\begin{align}
  ...
  W_{b,t} &= 
            ...
            \end{cases}
\end{align}
where $B$ is the set of all battles in the data, see Table \ref{tab:battles}.
...
News of battles is assumed to reach the market on the days the first news of the battle is reported in the \textit{New York Times} and the following \Sexpr{data_m2[["LAG"]]} days.
...
As an example, in this model the effect of the Battle of Gettysburg, $\omega_{\Sexpr{n_battle_gettysburg}}$, is shrunk towards both the average effect of Union victories, $\beta_{U}$, and the effect of the previous battle, $\omega_{\Sexpr{n_battle_gettysburg - 1}}$ (\Sexpr{n_battle_gettysburg_previous} on May 16, 1863).%
...
\begin{table}
  \centering
  \input{\Sexpr{file.path(tab_path, "avg_battle_result.tex")}}
...
Table \ref{tab:avg_battle_effects} shows the summary statistics of the posterior distribution of the average effects of Confederate and Union victories for the models.
...
Figure~\ref{fig:btl_effects} plots the battle effects, $\omega_{b}$, for each battle, for each model.
Figure~\ref{fig:btl_effects_ordered} plots the battle effects for model $\Model{3}$, for each battle.
Since the battle effects in model $\Model{4}$ can be considered time-varying values of effect of Confederate and Union victories, Figure~\ref{fig:btl_effects_time} plots the battle effects for Confederate and Union victories over time. 
...
\begin{figure}[!htpb]
  \centering
  \includegraphics{\Sexpr{file.path(fig_path, "fig_btl_effects-1")}}
  ...
  \label{fig:btl_effects}
\end{figure}

\begin{figure}[!htpb]
  \centering
  \includegraphics{\Sexpr{file.path(fig_path, "fig_btl_effects_ordered-1")}}  
  ...
  \label{fig:btl_effects_ordered}
\end{figure}

\begin{figure}[!htpb]
  \centering
  \includegraphics{\Sexpr{file.path(fig_path, "fig_btl_effects_time-1")}}  
  \caption{Battle effects, $\omega_{b}$, for all battles for model $\Model{4}$, by battle result.}
  \label{fig:btl_effects_time}
\end{figure}
...
Table \ref{tab:waic} shows the RMSE, WAIC, and expected log predictive density (elpd) of all the models.
...
The relatively small magnitude of the effects of individual battles is seen in Table \ref{tab:blt_effects_size}.
...
Considering that there are only \Sexpr{nrow(major_battles)} battles, but \Sexpr{as.integer(data_m2[["END_DATE"]] - data_m2[["START_DATE"]]) + 1L} values of $\eta_{t}$, neither of those magnitudes seems particularly large.
...
\begin{table}[!htpb]
  \centering
  \input{\Sexpr{file.path(tab_path, "tab_waic.tex")}}
...
\begin{table}[!htpb]
  \centering
  \input{\Sexpr{file.path(tab_path, "tab_btl_effects_size.tex")}}
  ...
  \label{tab:blt_effects_size}
\end{table}


% \begin{figure}
%   \centering
%   \includegraphics{\Sexpr{file.path(fig_path, "cum_btl_effects-1")}}
%   \caption{Cumulative effects of battles on the yield.}
% \end{figure}

% \begin{figure}
%   \centering
%   \includegraphics{\Sexpr{file.path(fig_path, "theta_star-1")}}
...
On its own, its results are limited mainly to questions of this particular war. 
Combined with similar anlyses across multiple wars, it may make clear patterns and 
...
%  LocalWords:  40's 30's Ball1991 Treasury1861a Treasury1861b 20's
...
%  LocalWords:  Arnold2015b Phisterer1883 dyer1908 cwsac2012 sui 40's
%  LocalWords:  KennedyConservation1998  MCMC Stan2015a generis 30's
...
%  LocalWords:  Clausewitz1989 Gartner1998 NorthWeingast1989 uring
